...
void inicializarTablero (int tablero[tam][tam] , int columnaInicial , int filaInicial){
    int i,j;
    for(i = 0 ; i < tam ; i++){
        for(j=0 ; j < tam ; j++){
            tablero[i][j] = 0;
...
//--------------------------------------------------------------------------------------------------------------------------------------------
//las agrego aca pq no recuerdo como predefinirla
bool tableroEstaSinTerminar(int tablero[tam][tam]){ //esta sin terminar cuando alguna de sus celdas esta igual a cuando lo inicializamos (= 0)
    int i,j;
    for(i = 0 ; i < tam ; i++){
        for(j=0 ; j < tam ; j++){
            if(tablero[i][j] == 0){
...
bool puedeSaltarCaballo(int tablero[tam][tam] , int nuevaColumna , int nuevaFila){
...
void pasoBacktracking (int tablero[tam][tam] , int columnaActual , int filaActual , int numeroSalto){
    ...
    int posiblesMovimientosColumna[]    = {  2, 2,  1, 1, -1, -1, -2, -2 };
    int posiblesMovimientosFila[]       = { -1, 1, -2, 2,  2, -2, -1,  1 };
...
/* PARA QUE LAS RECONOZCA LA FUNCION ANTERIOR
bool puedeSaltarCaballo(int tablero[tam][tam] , int nuevaColumna , int nuevaFila){
    return ((tablero[nuevaColumna][nuevaFila] == 0) && (nuevaColumna < tam && nuevaColumna >= 0) && (nuevaFila < tam && nuevaFila >= 0));
}
...
void mostrarTablero (int tablero[tam][tam]){ 
    int i,j;
    for(i = 0 ; i < tam ; i++){
        for(j=0 ; j < tam ; j++){
            cout << tablero[i][j]<< " | ";
